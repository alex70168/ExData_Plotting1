#This is the project 1-4 - multiple plot from 2007/02/01 to 2007/02/02
...
# Multiply plot
# set the multiplot par
par(mfrow=c(2,2))

# topleft
plot(strptime(paste(sub$Date,sub$Time),"%d/%m/%Y %H:%M:%S"),sub$Global_active_power,ylab="Global Active Power (kilowatts)",type="l",xlab="")
# topright
plot(strptime(paste(sub$Date,sub$Time),"%d/%m/%Y %H:%M:%S"),sub$Voltage,ylab="Voltage",type="l",xlab="datetime")
# bottomleft
...
# bottomright
plot(strptime(paste(sub$Date,sub$Time),"%d/%m/%Y %H:%M:%S"),sub$Global_reactive_power,ylab="Global_reactive_power",xlab="datetime",type="l")
dev.off()
